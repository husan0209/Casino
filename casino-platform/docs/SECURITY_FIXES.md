...
| 12 | ~~KYC `mimetype` доверяет клиентскому `Content-Type` (нет magic-byte проверки)~~ | `apps/api/src/modules/kyc/presentation/controllers/kyc.controller.ts` | ✅ Исправлено 2026-08-30: `memoryStorage` (недоверенный контент на диск не попадает) + `file-sniffer.ts` по сигнатурам JPEG/PNG/WebP/PDF; файл пишется только после sniff, имя = UUID + расширение от sniffed-типа, в БД пишется sniffed-тип; 8 тестов (в т.ч. PNG под видом JPEG, GIF/HTML/ZIP-отказы, обрезанные буферы). Остаток: тот же паттерн для аватаров `users.controller.ts` (следующий PR) |
