...
| 11 | ~~Frontend хранит access-token в localStorage + CSP `unsafe-inline/eval`~~ | `apps/web/src/stores/auth.ts`, `apps/web/src/lib/api.ts`, `apps/web/src/middleware.ts`, `infra/nginx/conf.d/casino.conf` | ✅ Исправлено 2026-08-30: `persist` убран — access-token и user только в памяти (XSS-кража localStorage более невозможна); silent-refresh single-flight по 401 + повтор исходного запроса (`_retry`, /auth/* исключены); `hydrate()` при загрузке: `/auth/refresh` по httpOnly-cookie → `/users/me`; logout чистит серверную cookie. **Остаток закрыт 2026-08-31 (PR #15): nonce-CSP через Next middleware** — на каждый запрос nonce + 'strict-dynamic' (Next сам проставляет nonce в hydration-скрипты); 'unsafe-eval' только в dev; nginx CSP для веб-хоста убран (двойной CSP пересекается браузером и ломает strict-dynamic), CSP для /api — helmet |
...
| 14 | ~~Тестов почти нет; деньги не покрыты~~ | `apps/api` | ✅ Закрыто 2026-08-31 (GAP-24): `money-flow.spec.ts` (11); `ledger.integration.spec.ts` (6, реальный Postgres в CI через `prisma db push` + `LEDGER_INTEGRATION=1`): idempotency, InsufficientFunds, **откат tx при сбое** и bet/win-сценарий в одной транзакии — проверено на настоящем Serializable-Postgres; `account-lockout.spec.ts` (5); `logger-redact.spec.ts` (3); `nowpayments-ipn.spec.ts` (13); `kyc-file-sniffer.spec.ts` (8); **E2E (GAP-05) — `player-lifecycle.e2e.spec.ts` (9): register→login→KYC→депозит (HMAC-вебхук)→launch→Bet/Win→withdrawal→admin-approve против собранного сервера в CI** |
...
| 16 | ~~`toMoney(n: any)` + 34 `as any` + глубокие относительные импорты~~ | `wallet.ledger.prisma.ts` и др. | ✅ Исправлено 2026-08-31: **0 `as any` в `apps/api/src`** (было 34). Деньги: `toMoney(Prisma.Decimal\|…)`, tx-клиенты `Prisma.TransactionClient`, `CreditInput.type: LedgerEntryType`, `metadata: Prisma.InputJsonValue`; enum-касты в kyc/support/payments/casino/admin — на типы enum'ов вместо any. Компилятор сразу поймал реальный runtime-баг: referral-награда писала `type: 'referral_reward'` вне enum. Относительные импорты — GAP-26 закрыт 2026-09-01: 72 глубоких импорта переведены на алиасы `@/` + `@modules/*`, prod-рантайм не пострадал благодаря `tsc-alias` в скрипте build (алиасы переписываются в относительные пути уже в `dist`, поэтому `node dist/main.js` работает без tsconfig-paths) |
