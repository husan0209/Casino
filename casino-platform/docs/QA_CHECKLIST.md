...
> **Сводка покрытия (GAP-45 от 2026-09-03; обновлено 2026-09-16 после GAP-52/53/54/55):** 35 пункта. **10 закрыто автотестами полностью** (`[x]` + `[auto: файл::тест]`); **9 закрыто частично** (`[x*]` — код-путь покрыт тестом, хвост боевой интеграции/UI помечен `[manual: …]` и ведёт в GAP-46); **16 — только ручные.** Автобаза: **194 unit + 9 E2E** в `apps/api/test/` (CI: `Tests 194 passed | 9 skipped`) и **151 unit + 6 DOM** в `apps/web/test/` (CI до #88: `Tests 153 passed`; web-спеки в чеклист добавлены как `[auto: web …]` у новых пунктов).
>
...
- [x*] Change password из профиля: старый пароль проверяется, прочие сессии revoke'ятся, OAuth-аккаунт без пароля → PASSWORD_NOT_SET — код-путь: `[auto: change-password.spec.ts::«GAP-52 ChangePasswordUseCase» (4 теста, UI-часть в profile/Безопасность)]`; живой HTTP-вызов — часть GAP-46
- [x*] Капча после 5 неудачных входов (§5.2) — код-путь: `[auto: captcha.spec.ts::«GAP-55 CaptchaService» (9 тестов: порог, отсутствие токена, fail-open при сетевом сбое, success=false ⇒ CAPTCHA_FAILED)]` + `[auto: web auth-store-captcha.spec.ts (4: captcha_token не шлётся пустым)]`; `[manual: живой siteverify Cloudflare Turnstile + виджет в браузере — нужен публичный HTTPS-домен и ключи, GAP-46]`
