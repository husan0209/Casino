...
> **Осознанное дополнение к ТЗ (GAP-50, согласовано владельцем 2026-09-04):**
> помимо перечисленного ниже, в API интегрирован Sentry-агрегатор ошибок
> (`@sentry/node`, точная версия 8.49.0). Отклонение от §12.1 зафиксировано
> здесь, как требует критерий 5 GAP-50. Контракт: `SENTRY_DSN` опционален —
> без него Sentry не инициализируется (no-op для dev/CI); трейсинг выключен
> (`tracesSampleRate: 0`); PII-скраббинг тем же списком, что pino redact
> (`SECURITY_BASELINE.md` §12.3); уходят только необработанные исключения и
> 5xx — бизнес-ошибки (AppError) и 4xx не шумят. Реализация:
> `apps/api/src/common/sentry/sentry.options.ts`, спек `test/sentry-options.spec.ts`.
