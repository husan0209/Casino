...
| 2026-09-03 | GAP-48 закрыт: снято последнее в репо inline-подавление `max-lines-per-function` (оказалось ненужным с первого коммита — 23 зачётные строки при лимите 90), из `runDaily` вынесен `dayBounds`, добавлен guard **G14** (запрет возвращать подавление). Попутно исправлена строка §2.1: она до сих пор обещала «временно 90», хотя GAP-30 вернул порог к 60 | AI agent (GAP-48) |
