...
| GAP-24 | A5, A6 | ~~Покрытие тестами: минимум — money flow + idempotency~~ | `apps/api` | 🔶 P2 частично закрыт 2026-08-30: `money-flow.spec.ts` (11 тестов — idempotency, rollback-типы bet→credit/win→debit, атомарность bet/win/rollback), `account-lockout.spec.ts` (5), `logger-redact.spec.ts` (3); осталось E2E + NOWPayments IPN + DB-интеграционные |
