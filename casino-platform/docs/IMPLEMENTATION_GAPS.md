...
7. ~~После MVP: GAP-22/24/25/26~~ **2026-09-01: закрыты все четыре** — GAP-22 (4-слойка wallet), GAP-24 (тесты + E2E), GAP-25 (ESLint error-пороги 3/10), GAP-26 (алиасы через `tsc-alias`); из P2/P3-трекера остаётся только GAP-28 (defense-in-depth идемпотентности депозита) и GAP-30 (возврат `max-lines-per-function` к 60); плюс email-воркер отдельным процессом и rich HTML-шаблоны.
