...
| GAP-09 | Admin sync-gages — заглушка | ⚠️ Осталось: в `casino-admin.controller.syncGames` вызвать `adapter.fetchGameList()` и upsert каталога (адаптер уже отдаёт нормализованный список) |
