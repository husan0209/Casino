...
8. **Аудит готовности 2026-09-01 → GAP-31…GAP-38** (раздел выше). Порядок работ:
   **сначала P0** — ~~GAP-31~~ (закрыт, PR #26) и ~~GAP-32~~ (закрыт 2026-09-02: cron + admin-эндпоинт +
   интеграционный тест); **затем P1** — ~~GAP-33~~ (закрыт 2026-09-02: BullMQ Job Schedulers), GAP-34
   (курсы из БД — таблица и Redis-кеш уже пишутся job'ой `update-rates`, остался переключатель
   потребителей), GAP-35 (честный readiness); **потом P2/P3** — GAP-36, GAP-38, GAP-37, GAP-30.
