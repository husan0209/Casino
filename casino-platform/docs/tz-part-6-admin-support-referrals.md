...
```text
ID | Email | Статус | KYC | Баланс (RUB экв. reporting) | Депозиты | Выводы | GGR | Дата рег. | Послед. вход | Действия
```

> **Reporting vs игрок:** колонка «RUB экв.» — только для admin-сортировки и отчётности (внутренние курсы, см. [tz-part-3](tz-part-3-payments-wallet.md) §8). На публичном фронте общий баланс в рублях **не показывается** ([tz-part-5](tz-part-5-frontend-web.md) §2.3). В карточке пользователя — таблица балансов **по каждой валюте отдельно**.
