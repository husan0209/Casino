...
last_updated: 2026-08-28 (snapshot, не редактируется)
---

> ## ⛔ SNAPSHOT — документ не редактируется
>
> **Аудит проведён:** 2026-08-25. **Ревизия статусов:** 2026-08-28 (сверка каждого пункта с кодом).
> Единственное место, где живут актуальные статусы задач: **`docs/IMPLEMENTATION_GAPS.md`** (GAP-18..28).
> Честный статус security-чеклиста: **`docs/SECURITY_CHECKLIST.md`**.
>
> ### Ревизия 2026-08-28 — что стало с каждым пунктом
>
> | ID | Статус | Подтверждение / куда |
> |----|--------|----------------------|
> | N1 | ✅ исправлено | `register.dto.ts:6-10` — `min(8)` + `regex(/\d/)` |
> | N2 | ❌ открыт | account lockout не реализован → **GAP-18** |
> | N3 | ❌ открыт | `@nestjs/throttler` отсутствует → **GAP-19** |
> | N4 | ❌ открыт | `helmet` отсутствует → **GAP-20** |
> | N5 | ✅ исправлено | `main.ts` — `bodyParser({verify})` + `req.rawBody`; вебхук-контроллер передаёт `rawBody` в use-case |
> | N6 | ✅ исправлено | `kyc.controller.ts:67` — `fileFilter` |
> | N7 | ✅ исправлено | `common/cookies/refresh-token-cookie.ts:18` — `secure: isProduction` |
> | N8 | ❌ открыт | `auth.controller.ts:79` — `@Body() body: { email: string }` без Zod → **GAP-21** |
> | N9 | ❌ открыт | `payments.controller.ts` — инлайн-типы без Zod → **GAP-21** |
> | N10 | ✅ исправлено | `env.validation.ts:93` — `isUnsafeSecret(JWT_ACCESS_SECRET)` |
> | N11 | ✅ исправлено (prod) | `USER node` во всех `*.prod.Dockerfile`; dev-образы root by design — `infra/docker/README.md` |
> | N12 | ✅ исправлено | `global-exception.filter.ts` — ветка для `message: string \| string[]` |
> | H1 | ✅ исправлено | `wallet.ledger.prisma.ts` — retry ×3 (`:95`) + `isolationLevel: 'Serializable'` (`:121`) |
> | H2 | ✅ исправлено | `:145` — guard `UNLOCK_EXCEEDS_LOCKED` |
> | H3 | ✅ исправлено | `classifyPaymentStatus()` в `domain/payment-status` вместо `status.includes` |
> | H4 | ⚠️ открыт (P3) | `'deposit_' + pr.id` без изменений → **GAP-28** |
> | H5 | ✅ исправлено | прямых вызовов `prisma` в `login.use-case.ts` нет |
> | H6 | ❌ открыт | pino не подключён → **GAP-23** |
> | H7 | ✅ исправлено | `request-id.middleware.ts` — whitelist `REQUEST_ID_PATTERN` |
> | C1 | ✅ исправлено | `@ts-ignore` в `main.ts` нет |
> | C2 | ✅ исправлено | `console.log` в `main.ts` нет |
> | C3 | ❌ открыт | → **GAP-21** (вместе с N8/N9) |
> | C4 | ⚠️ открыт | 18 относительных импортов `../../../../` → **GAP-26** |
> | C5 | ❌ открыт | `toMoney(n: any)` — `wallet.ledger.prisma.ts:21` → **GAP-22** |
> | C6 | ❌ открыт | `runCreditDebit` ~50 строк → **GAP-22** |
> | A1 | ❌ открыт | в `wallet/application/` только `wallet.facade.ts`, `use-cases/` нет → **GAP-22** |
> | A2 | ✅ исправлено | dynamic import из `wallet.facade.ts` убран |
> | A3 | ✅ закрыт | use-case использует `PaymentRequestRepository` (абстракция), не сырой prisma |
> | A4 | ✅ подтверждено | `WalletFacade`/`UsersFacade` через DI |
> | A5 | ❌ открыт | 2 spec-файла на монорепо → **GAP-24** |
> | A6 | ❌ открыт | следует из A5 → **GAP-24** |
> | A7 | ⚠️ частично | `no-console/no-explicit-any/no-cycle/max-depth/max-lines-per-function` → error; `max-params` (warn, 4) и `complexity` (warn, 10) остались → **GAP-25** |
> | NEW | ❌ новая находка 2026-08-28 | **argon2 без явных параметров**: `password-hasher.service.ts:7` — `argon2.hash(plain, {type: argon2id})`, дефолты ~19MiB/2/1 вместо `SECURITY_BASELINE.md §2.1` (65536/3/4) → **GAP-27** |
>
> **Сводка ревизии:** 17 исправлено · 13 открыты (GAP-18..28) · процессные пункты §6–7 — в `BRANCH_PROTECTION.md` (branch protection включается вручную в GitHub UI; pre-commit хуки на FAT/sdcard по-прежнему не работают — см. `.husky/pre-push` SKIP-логику).
...
>
> **2026-08-28:** пути к wallet-репозиторию обновлены (`infrastructure/repositories/wallet.prisma.ts` → `infrastructure/ledger/wallet.ledger.prisma.ts` — файл переименован после аудита). Часть пунктов уже исправлена в коде — сверяй со статусом в `IMPLEMENTATION_GAPS.md`; полная ревизия списка — отдельная задача (Фаза 1).
...
- **Файл:** `apps/api/src/modules/wallet/infrastructure/ledger/wallet.ledger.prisma.ts` (метод `lock`)
...
- **Файл:** `apps/api/src/modules/wallet/infrastructure/ledger/wallet.ledger.prisma.ts` (метод `unlock`)
...
- **Файл:** `apps/api/src/modules/wallet/infrastructure/ledger/wallet.ledger.prisma.ts:9`
...
- **Файл:** `apps/api/src/modules/wallet/infrastructure/ledger/wallet.ledger.prisma.ts` метод `runCreditDebit` (~50 строк)
...
- **Следствие:** `lock/unlock/confirmWithdrawal` живут прямо в `infrastructure/ledger/wallet.ledger.prisma.ts` — смешение application и infrastructure
