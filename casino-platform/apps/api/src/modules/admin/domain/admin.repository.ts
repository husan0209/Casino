 ...
 * Дашборд читает чужие агрегаты (users/payments/kyc/tickets) — это не долг, а
 * ответственность модуля: отчётность по всем доменам закреплена в §13.1
 * MODULE_BOUNDARIES («Dashboard metrics»), отдельного фасада-владельца у сводных
 * метрик нет; берутся только read-only агрегаты, не сырые деньги.
 * Поэтому мёртвая ссылка на закрытый GAP-22 (закрыт 2026-08-31 и про другое)
 * убрана. ADR GAP-51 (2026-09-04): read-only cross-module доступ через общий
 * Prisma-клиент — принятое решение всего репо; port-ификация — только при
 * выносе модулей в отдельные сервисы (решение владельца на тот момент).
 */
