...
export function Btn({ children, onClick, variant = 'primary', disabled, small }: {
  children: ReactNode; onClick?: () => void; variant?: BtnVariant; disabled?: boolean; small?: boolean
}) {
  return (
...
export function Input({ small, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { small?: boolean }) {
  return <input {...props} className={`input ${small ? 'text-xs px-2 py-1' : ''} ${props.className ?? ''}`} />
}
...
const badgeColors: Record<string, string> = {
  completed: 'bg-emerald-900/60 text-emerald-300', approved: 'bg-emerald-900/60 text-emerald-300', credited: 'bg-emerald-900/60 text-emerald-300', active: 'bg-emerald-900/60 text-emerald-300',
  pending: 'bg-yellow-900/60 text-yellow-300', in_progress: 'bg-blue-900/60 text-blue-300',
  cancelled: 'bg-zinc-700 text-zinc-300', closed: 'bg-zinc-700 text-zinc-300', blocked: 'bg-red-900/60 text-red-300',
  rejected: 'bg-red-900/60 text-red-300', failed: 'bg-red-900/60 text-red-300', expired: 'bg-zinc-700 text-zinc-300',
  open: 'bg-sky-900/60 text-sky-300', waiting_user: 'bg-purple-900/60 text-purple-300',
}
...
export function Pager({ page, perPage, total, onPage }: { page: number; perPage: number; total: number; onPage: (p: number) => void }) {
  ...
  return (
    <div className="flex items-center gap-3 mt-3 text-sm text-[#8b8ba7]">
      <Btn small variant="ghost" disabled={page <= 1} onClick={() => onPage(page - 1)}>← Назад</Btn>
      <span>{page} / {pages} · всего {total}</span>
      <Btn small variant="ghost" disabled={page >= pages} onClick={() => onPage(page + 1)}>Вперёд →</Btn>
    </div>
  )
}
...
export function Th({ children }: { children?: ReactNode }) {
  return <th className="text-left text-xs uppercase tracking-wide text-[#8b8ba7] px-3 py-2 border-b border-white/10">{children}</th>
}
