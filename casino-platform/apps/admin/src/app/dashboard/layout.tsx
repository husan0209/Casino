    ...
    <div className="min-h-screen bg-[#0b0b12] text-white">
      <header className="h-14 border-b border-white/10 bg-[#141420] flex items-center px-4 justify-between">
        ...
        <div className="flex items-center gap-3 text-sm text-[#8b8ba7]">
          <span>{admin?.email ?? 'admin'} <span className="text-[10px] uppercase text-[#ff3b7a]">{admin?.role}</span></span>
          <button onClick={() => { logout(); router.replace('/') }} className="rounded-lg bg-white/5 hover:bg-white/10 px-3 py-1.5">Выйти</button>
        </div>
      </header>
      <div className="flex">
        <aside className="w-60 border-r border-white/10 min-h-[calc(100vh-56px)] p-3">
          <nav className="space-y-1 text-sm">
            {nav.map(([label, href]) => (
              <Link key={href} href={href} className="block px-3 py-2 rounded-lg hover:bg-white/5">{label}</Link>
            ))}
          </nav>
        </aside>
