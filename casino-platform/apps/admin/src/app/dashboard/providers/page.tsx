...
export default function ProvidersPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['providers'], queryFn: () => apiGet<ProviderRow[]>('/admin/providers') })
  ...
  return (
    <div>
      ...
      <ErrorBox msg={err} />
      {note && <div className="mb-3 rounded-lg bg-blue-950/60 border border-blue-800 px-3 py-2 text-sm text-blue-300">{note}</div>}
      {isLoading && <Loading />}
      {data && (
        <table className="w-full text-sm">
          <thead><tr><Th>Название</Th><Th>Slug</Th><Th>Тип</Th><Th>Игр</Th><Th>Статус</Th><Th>Действия</Th></tr></thead>
          <tbody>
            {data.map((p) => (
              <tr key={p.id}>
                <Td>{p.name}</Td>
                ...
                <Td><Badge value={p.isEnabled ? 'active' : 'cancelled'} /></Td>
                <Td>
                  <div className="flex gap-1 flex-wrap">
                    <Btn small variant={p.isEnabled ? 'ghost' : 'ok'} onClick={() => act.mutate({ id: p.id, action: p.isEnabled ? 'disable' : 'enable' })}>{p.isEnabled ? 'Выключить' : 'Включить'}</Btn>
                    <Btn small variant="ghost" onClick={() => act.mutate({ id: p.id, action: 'sync-games' })}>🔄 Синхронизировать</Btn>
                  </div>
                </Td>
              </tr>
            ))}
