...
export default function TransactionsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiGetFull<LedgerRow[]>('/admin/transactions', { page, per_page: 30, type: type || undefined }),
    ...
    <div>
      ...
      <div className="flex gap-3 mb-4">
        <Select value={type} onChange={(e) => { setType(e.target.value); setPage(1) }} className="w-56">
          ...
          {['DEPOSIT','WITHDRAWAL','WITHDRAWAL_LOCK','WITHDRAWAL_UNLOCK','WITHDRAWAL_CONFIRM','BET','WIN','ROLLBACK','ADMIN_CREDIT','ADMIN_DEBIT','REFERRAL_REWARD'].map(t => <option key={t} value={t}>{t}</option>)}
        </Select>
      </div>
      ...
      {data?.data && (
        <>
          <table className="w-full text-sm">
            <thead><tr><Th>Дата</Th><Th>Пользователь</Th><Th>Тип</Th><Th>Сумма</Th><Th>Валюта</Th><Th>Описание</Th></tr></thead>
            <tbody>
              {data.data.map((t) => {
                ...
                return (
                  <tr key={t.id}>
                    ...
                    <Td className={`font-mono ${plus ? 'text-emerald-400' : 'text-red-400'}`}>{plus ? '+' : '−'}{Number(t.amount).toLocaleString('ru-RU')}</Td>
