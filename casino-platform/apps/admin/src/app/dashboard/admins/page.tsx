...
export default function AdminsPage() {
  ...
  const list = useQuery({ queryKey: ['admins'], queryFn: () => apiGet<AdminRow[]>('/admin/admins'), enabled: isSuper })
  ...
  return (
    <div>
      <PageTitle>Администраторы</PageTitle>
      <ErrorBox msg={err} />
      {ok && <div className="mb-3 rounded-lg bg-emerald-950/60 border border-emerald-800 px-3 py-2 text-sm text-emerald-300">{ok}</div>}

      {list.isLoading && <Loading />}
      {list.data && (
        <table className="w-full text-sm mb-8">
          <thead><tr><Th>Email</Th><Th>Имя</Th><Th>Роль</Th><Th>Статус</Th><Th>Последний вход</Th><Th></Th></tr></thead>
          <tbody>
            {list.data.map((a) => (
              <tr key={a.id}>
                <Td>{a.email}</Td>
                <Td className="text-[#8b8ba7]">{[a.firstName, a.lastName].filter(Boolean).join(' ') || '—'}</Td>
      ...
      <form className="flex gap-2 flex-wrap items-end max-w-3xl" onSubmit={(e) => { e.preventDefault(); create.mutate() }}>
        <Input type="email" placeholder="Email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} required className="w-52" />
        <Input type="password" placeholder="Пароль" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} required minLength={8} className="w-44" />
        <Input placeholder="Имя" value={form.first_name} onChange={e => setForm({ ...form, first_name: e.target.value })} className="w-32" />
        <Input placeholder="Фамилия" value={form.last_name} onChange={e => setForm({ ...form, last_name: e.target.value })} className="w-36" />
        <Select value={form.role} onChange={e => setForm({ ...form, role: e.target.value as any })} className="w-36">
