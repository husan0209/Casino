...
export default function AuditPage(): React.JSX.Element {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiGetFull<AuditRow[]>('/admin/audit-logs', {
        page,
        per_page: 30,
            ...
            <tbody>
              {data.data.map((a) => (
                <tr key={a.id}>
                  ...
                  <Td>
                    {a.actorType}
                    <div className="text-xs text-[#8b8ba7] font-mono">{a.actorId.slice(0, 8)}</div>
                  </Td>
                  <Td className="font-mono text-xs">{a.action}</Td>
                  <Td className="text-[#8b8ba7] text-xs">
                    {a.targetType ?? '—'}
                    <div className="font-mono">{a.targetId?.slice(0, 8) ?? ''}</div>
                  </Td>
                  <Td className="text-[#8b8ba7] text-xs font-mono">{a.ipAddress ?? '—'}</Td>
                  <Td>
                    {a.payload ? (
                      <details>
                        <summary className="cursor-pointer text-xs text-[#60a5fa]">детали</summary>
                        <pre className="mt-1 max-w-sm overflow-x-auto text-[10px] text-[#8b8ba7]">
