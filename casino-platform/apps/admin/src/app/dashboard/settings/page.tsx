...
export default function SettingsPage(): React.JSX.Element {
  ...
  const list = useQuery({
    queryKey: ['settings'],
    queryFn: () => apiGet<SettingRow[]>('/admin/settings'),
  })
  ...
  return (
    <div>
      ...
      {!isSuper && (
        <div className="mb-4 text-sm text-yellow-300">Изменение доступно только superadmin.</div>
      )}
              ...
              <tr key={row.id}>
                ...
                <Td>
                  {isSuper && drafts[row.key] !== undefined && drafts[row.key] !== row.value && (
                    <Btn small onClick={() => save.mutate(row)} disabled={save.isPending}>
                      Сохранить
                    </Btn>
                  )}
                </Td>
