...
interface Metrics {
  ...
  finance: { deposits: string; withdrawals: string; ggr: string; deposits_total: string; withdrawals_total: string }
  pending: { withdrawals: number; kyc: number; tickets: number }
}
type Period = 'today' | '7d' | '30d' | '90d'

const fmt = (v: string | number) => typeof v === 'number' ? v.toLocaleString('ru-RU') : `${Number(v).toLocaleString('ru-RU')} ₽`

export default function DashboardPage() {
  const [period, setPeriod] = useState<Period>('today')
  const [chartDays, setChartDays] = useState<Exclude<Period, 'today'>>('7d')

  const metrics = useQuery({ queryKey: ['dash', 'metrics', period], queryFn: () => apiGet<Metrics>('/admin/dashboard/metrics', { period }), refetchInterval: 30000 })
  const revenue = useQuery({ queryKey: ['dash', 'revenue', chartDays], queryFn: () => apiGet<{ labels: string[]; datasets: Record<string, string[]> }>('/admin/dashboard/charts', { period: chartDays, type: 'revenue' }) })
  const regs = useQuery({ queryKey: ['dash', 'regs', chartDays], queryFn: () => apiGet<{ labels: string[]; datasets: { registrations: number[] } }>('/admin/dashboard/charts', { period: chartDays, type: 'registrations' }) })
  const events = useQuery({ queryKey: ['dash', 'events'], queryFn: () => apiGet<Array<{ at: string; type: string; detail: string }>>('/admin/dashboard/events', { limit: 10 }) })
    ...
    <div>
      ...
      {metrics.data && (
        <div className="flex gap-3 mb-8 text-sm">
          <Link href="/dashboard/withdrawals?status=pending" className="bg-[#141420] border border-white/10 rounded-xl px-4 py-2 hover:bg-white/5">
            ⏳ Выводы на модерации: <b className="text-yellow-300">{metrics.data.pending.withdrawals}</b>
          </Link>
          <Link href="/dashboard/kyc?status=pending" className="bg-[#141420] border border-white/10 rounded-xl px-4 py-2 hover:bg-white/5">
            🪪 KYC на проверке: <b className="text-yellow-300">{metrics.data.pending.kyc}</b>
          </Link>
          <Link href="/dashboard/support?status=open" className="bg-[#141420] border border-white/10 rounded-xl px-4 py-2 hover:bg-white/5">
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        {[['7d', '7 дней'], ['30d', '30 дней'], ['90d', '90 дней']].map(([v, l]) => (
          <button key={v} onClick={() => setChartDays(v as Exclude<Period, 'today'>)}
            className={`rounded-lg px-3 py-1 text-xs ${chartDays === v ? 'bg-[#ff3b7a]' : 'bg-white/5 hover:bg-white/10'}`}>{l}</button>
        ))}
      </div>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={revData}>
              ...
              <Legend />
              <Line type="monotone" dataKey="deposits" name="Депозиты" stroke="#34d399" dot={false} />
              <Line type="monotone" dataKey="withdrawals" name="Выводы" stroke="#f87171" dot={false} />
            ...
            <tbody>
              {events.data.map((e, i) => (
                <tr key={i} className="border-b border-white/5">
                  <td className="px-2 py-1.5 text-[#8b8ba7] whitespace-nowrap">{new Date(e.at).toLocaleTimeString('ru-RU')}</td>
                  <td className="px-2 py-1.5"><span className="text-xs bg-white/10 rounded px-1.5 py-0.5">{e.type}</span></td>
