...
export default function UsersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiGetFull<UserRow[]>('/admin/users', {
        page,
        per_page: 20,
  ...
  const act = useMutation({
    mutationFn: ({ id, action }: { id: string; action: 'block' | 'unblock' }) =>
      apiPost(`/admin/users/${id}/${action}`, { reason: 'admin panel' }),
    onSuccess: () => {
      setErr(undefined)
                  ...
                  <Td>
                    {u.status === 'blocked' ? (
                      <Btn
                        small
                        variant="ok"
                        onClick={() => act.mutate({ id: u.id, action: 'unblock' })}
                      >
                        Разблокировать
                      </Btn>
                    ) : (
                      <Btn
                        small
                        variant="danger"
                        onClick={() => act.mutate({ id: u.id, action: 'block' })}
                      >
                        Блокировать
                      </Btn>
                    )}
