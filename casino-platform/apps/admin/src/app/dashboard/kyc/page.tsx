...
export default function KycPage(): React.JSX.Element {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiGetFull<KycRow[]>('/admin/kyc', { page, per_page: 20, status: status || undefined }),
      ...
      <div className="flex gap-2 mb-4 text-sm">
        {(
          [
            ...
            ['approved', 'Одобрены'],
            ['rejected', 'Отклонены'],
            ['requires_resubmission', 'Повторная отправка'],
          ...
          <button
            ...
            className={`rounded-lg px-3 py-1.5 ${status === v ? 'bg-[#ff3b7a]' : 'bg-white/5 hover:bg-white/10'}`}
          >
            {l}
          </button>
        ))}
              ...
              {data.data.map((k) => (
                <tr key={k.id}>
                  ...
                  <Td className="text-[#8b8ba7]">
                    {k.submittedAt ? new Date(k.submittedAt).toLocaleString('ru-RU') : '—'}
                  </Td>
                  <Td>
                      ...
                      (rejectId === k.id ? (
                        <div className="flex gap-1 items-center">
                          ...
                          <Btn
                            small
                            variant="danger"
                            onClick={() => act.mutate({ id: k.id, action: mode })}
                          >
                            OK
                          </Btn>
                        ...
                        <div className="flex gap-1">
                          <Btn
                            small
                            variant="ok"
                            onClick={() => act.mutate({ id: k.id, action: 'approve' })}
                          >
                            Одобрить
                          </Btn>
                          <Btn
