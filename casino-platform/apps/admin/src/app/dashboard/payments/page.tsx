...
export default function PaymentsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiGetFull<PaymentRow[]>('/admin/payment-requests', {
        page,
        per_page: 30,
    ...
    <div>
      ...
      <div className="flex gap-3 mb-4">
        ...
        >
          ...
          {['pending', 'processing', 'completed', 'failed', 'cancelled', 'expired'].map((s) => (
        ...
        >
          ...
          {['rukassa', 'nowpayments', 'manual'].map((p) => (
            ...
            <tbody>
              {data.data.map((p) => (
                <tr key={p.id}>
                  ...
                  <Td className="text-[#8b8ba7] font-mono text-xs max-w-[140px] truncate">
                    {p.externalId ?? '—'}
                  </Td>
                </tr>
              ))}
