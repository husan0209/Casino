...
export default function SupportPage(): React.JSX.Element {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      apiGetFull<TicketRow[]>('/admin/support/tickets', {
        page,
        per_page: 20,
  ...
  const act = useMutation({
    mutationFn: ({ id, action, body }: { id: string; action: string; body?: unknown }) =>
      action === 'messages'
        ? apiPost(`/admin/support/tickets/${id}/messages`, body)
        : apiPost(`/admin/support/tickets/${id}/${action}`, body),
    onSuccess: () => {
      setErr(undefined)
      setReply('')
    ...
    <div>
      ...
      {openId && (
        <div className="mt-6 bg-[#141420] border border-white/10 rounded-2xl p-4">
          ...
          <textarea
            className="input min-h-[70px] w-full mb-2"
            placeholder="Ответ…"
            value={reply}
          ...
          <div className="flex gap-2 flex-wrap items-center">
            <Btn
              ...
              onClick={() =>
                act.mutate({
                  id: openId,
                  action: 'messages',
                  body: { message: reply, is_internal: false },
                })
              }
              disabled={!reply.trim()}
            >
            ...
            <Btn
              ...
              onClick={() =>
                act.mutate({
                  id: openId,
                  action: 'messages',
                  body: { message: reply, is_internal: true },
                })
              }
              disabled={!reply.trim()}
            >
            ...
            <Btn
              small
              variant="ghost"
              onClick={() => act.mutate({ id: openId, action: 'assign', body: {} })}
            >
              Назначить на себя*
            </Btn>
            <Select
              ...
              onChange={(e) =>
                e.target.value &&
                act.mutate({ id: openId, action: 'priority', body: { priority: e.target.value } })
              }
            >
            ...
            <Btn small variant="danger" onClick={() => act.mutate({ id: openId, action: 'close' })}>
              Закрыть тикет
            </Btn>
          </div>
        </div>
